...
routers.post("/location", async (req: Request, res: Response) => {
  ...
  try {
      ...
      if (
        ...
        do {
          ...
          for (let i = 0; i < list.length; i++) {
            const card = list[i];
            const _card = card.card_group.filter(
              (card) => card.card_type === 9
            );
                ...
                if (
                  ...
                  if (userInfo && userInfo["more"]) {
                    ...
                    if (commentUsers.length > 2) {
                      ...
                      writeFileSync(
                        join(__dirname, `../data/${location}.txt`),
                        txt.join("\n\n")
                      );
          ...
          bot
            .sendMessage(
              process.env.GROUP_ID,
              `${location}任务: ${users.length}/${limit}`
            )
            .catch((err) => {});
    ...
    handleSendFile(location, users.length).catch(() => {});
    if (!res.headersSent) {
      res.json({
        success: true,
      });
